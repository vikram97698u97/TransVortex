...
if (fs.existsSync('add.html')) {
    ...
    content = content.replace(/              <thead>\s*<tr>\s*<th>Vehicle No\.<\/th>\s*<th>Type<\/th>\s*<th>EMI<\/th>\s*<th>PUC<\/th>\s*<th>Insurance<\/th>\s*<th>Fitness<\/th>\s*<th class="text-center">Actions<\/th>\s*<\/tr>\s*<\/thead>/g, tableHeaderFix);
    ...
    // Insert the image column before the Actions column
    content = content.replace(/           <td class="text-center">\s*<div class="action-btns">/g, vehicleImageDisplay + '<div class="action-btns">');
    
    // Fix 4: Also update the vehicle card view to show documents
    const vehicleCardDocumentsFix = `               <div class="mb-2">
                 <strong>Documents:</strong><br>
                 ${vehicle.pucFileUrl ? 
                   `<a href="${vehicle.pucFileUrl}" target="_blank" class="btn btn-sm btn-outline-primary me-1 mb-1">
                      <i class="fas fa-file-pdf"></i> PUC
                    </a>` : 
                   '<span class="text-muted small">No PUC</span>'}
                 ${vehicle.insuranceFileUrl ? 
                   `<a href="${vehicle.insuranceFileUrl}" target="_blank" class="btn btn-sm btn-outline-info me-1 mb-1">
                      <i class="fas fa-file-pdf"></i> Insurance
                    </a>` : 
                   '<span class="text-muted small">No Insurance</span>'}
                 ${vehicle.fitnessFileUrl ? 
                   `<a href="${vehicle.fitnessFileUrl}" target="_blank" class="btn btn-sm btn-outline-success me-1 mb-1">
                      <i class="fas fa-file-pdf"></i> Fitness
                    </a>` : 
                   '<span class="text-muted small">No Fitness</span>'}
               </div>`;
    
    // Add documents to vehicle card view (find a good spot before the actions)
    content = content.replace(/               <div class="mt-auto">\s*<div class="d-flex gap-2">/g, vehicleCardDocumentsFix + '<div class="mt-auto"><div class="d-flex gap-2">');
    
    // Fix 5: Add some CSS for better styling of the document buttons
    const cssFix = `    /* Extra styles for document buttons in table */
    .table .btn-sm {
        font-size: 0.75rem;
    ...
    content = content.replace(/    \/\* Loading animation \*\//g, cssFix);
    ...
    if (content !== fs.readFileSync('add.html', 'utf8')) {
        ...
        console.log('✅ Updated vehicle card view with documents');
